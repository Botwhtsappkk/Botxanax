...
prefix = "!"
name = "~  XANAX  | kk"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM TEMENTE  XANAX kk😁"
rmenu = " XANAX DOMINA KARAY:)"
limitt = 10
ban = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do  XANAX kk digite ${prefix}daftar`,
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do  XANAX kk:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				user.push(sender)
				myMonths = ["janeiro","fevereiro","Março","abril","Mei","Junho","Julho","agosto","Setembro","Outubro","Novembro","Dezembro"];
                myDays = ['Domigo','Segunda-feira','terça','Quarta feira','Quinta feira','Jum at','sábado'];
                var tgl = new Date();
                var day = tgl.getDate()
                  bulan = tgl.getMonth()
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium  XANAX kk``` :\n'
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return client.sendMessage(from, 'Qual código de idioma, mano?\n Saja Se você não sabe o código do idioma, basta digitar *${prefix}bahasa*', text, {quoted: mek})
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('5511934713306@s.whatsapp.net', options, text, {quoted: mek})
                    reply('O proprietário do BOT, solicitações falsas / main2 não serão respondidas.')
                    break
				case 'memeindo':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ marcando todo mundo fds *${pushname2}* ✪══`+ teks +'╚═══〘  XANAX kk 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    ...
			    reply(mess.wait)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`\`\`\`✓Sucesso grupo fechado\`\`\` *${groupMetadata.subject}*`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`\`\`\`✓Sucesso grupo aberto\`\`\` *${groupMetadata.subject}*`)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sucesso deleta todo o chat  XANAX kk\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * XANAX kk BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				if (!isOwner) return reply(mess.only.ownerB)
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('A tag do alvo que você deseja clonar')
...
//setting bot
				...
				case 'setmemlimit':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					limitt = args[0]
					reply(`Limit alterado com sucesso para : ${limitt}`)
					break 
				case 'setnamebot':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
					name = body.slice(12)
					reply(`Nome Bot alterado com sucesso para : ${name}`)
					break 
				case 'setreply':
					if (!isOwner) return reply(mess.only.ownerB)
                    client.updatePresence(from, Presence.composing) 
...
// premium user
       ...
       case 'ytmp4':
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *LINHA DO TEMPO PLAY MP3* 」\n*• Título:* ${anu.result.title}\n*• Fonte:* ${anu.result.source}\n*• Tamanho:* ${anu.result.size}\n\n*JA ESTOU ENVIANDO A MÚSICA CALMA AE MANIN*`
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ XANAX kk]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
